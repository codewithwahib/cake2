      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8">
        {watchData.map((watch) => (
          <div key={watch.id} className="bg-white p-4 rounded-lg shadow-lg">
            ...
            />
            <h3 className="text-xl font-semibold text-purple-800">{watch.name}</h3>
            <p className="text-gray-700 text-sm mb-4">{watch.description}</p>
            <div className="text-lg font-bold text-purple-700">${watch.price}</div>
            <button className="w-full mt-4 py-2 bg-purple-800 text-white rounded-md hover:bg-purple-700">
              Add to cart
            </button>
          </div>
        ))}
